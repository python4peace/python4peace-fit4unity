#!/bin/bash
# Fit4Unity Complete Upgrade Script
# Purpose: Fix errors, update UI, install dependencies, start app

# 1️⃣ Navigate to project folder
cd ~/fit4unity || { echo "Folder not found!"; exit 1; }
...
# 3️⃣ Upgrade pip and install all packages
pip install --upgrade pip setuptools wheel
pip install --upgrade -r requirements.txt
# Ensure PyAudio fallback installed
pip install pipwin || true
pip install pyaudio || echo "PyAudio may not work on Render or Linux server"

# 4️⃣ Backup current app.py
cp app.py app_backup_$(date +%Y%m%d_%H%M%S).py

# 5️⃣ Update app.py with fixes for microphone + home screen
cat > app.py << 'EOF'
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
import os
...
# 6️⃣ Start Streamlit safely
echo "🚀 Starting Streamlit..."
nohup streamlit run app.py --server.port 8501 --server.address 0.0.0.0 > streamlit_log.txt 2>&1 &

echo "✅ Upgrade complete! Visit your app at http://localhost:8501 or your Render link"
